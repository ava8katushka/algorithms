...
The key takeaway? Even if the input size explodes, the number of steps only creeps up.
...
Logarithmic time is like leveling up in a video game: each level (step) cuts the remaining work **in half**.
...
As you know, logarithms are fast. So this algorithm is just a little bit slower than your usual linear one.
For n = 10^9 n log n is 10^9 * log 10^9 = 10^9  * (3 log 10^3) = 30 ^ 10^9

This complexity is common in efficient sorts, like

1. Merge sort
2. Heap sort
3. Average case of quick sort

Example:

<Code>
{
`sorted(array) # using tim sort here`
}
</Code>
